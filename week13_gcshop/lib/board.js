...
module.exports = {
  ...
  view: (req, res) => {
    ...
    db.query(sql1 + sql2 + sql3, (error, results) => {
      ...
      db.query(
        `select b.board_id as board_id, b.title as title, b.date as date, 
                IFNULL(p.name, b.loginid) as name, 
                IFNULL(p.name, b.loginid) as loginid, 
                b.p_id as p_id
         from board b left join person p on b.loginid = p.loginid 
         where b.type_id = ? 
         ORDER BY IFNULL(b.p_id, b.board_id) desc, b.board_id asc 
         LIMIT ? OFFSET ?`,
        [sntzedTypeId, numPerPage, offs],
        (err, boards) => {
          if (err) throw err;
  ...
  create_process: (req, res) => {
    ...
    db.query(
      ...
      (error, result) => {
        ...
        db.query(
          `UPDATE board SET p_id = ? WHERE board_id = ?`,
  ...
  detail: (req, res) => {
    ...
    var sql1 = `SELECT * FROM boardtype; `;
    var sql2 = `
      SELECT b.board_id, b.type_id, b.loginid, p.name AS writer_name, b.title, b.content, b.date, b.p_id 
      FROM board b 
      LEFT JOIN person p ON b.loginid = p.loginid 
      WHERE b.board_id = ?; 
    `;
    var sql3 = `SELECT COUNT(*) as replyCount FROM board WHERE p_id = ? AND board_id != ?; `;
    ...
    db.query(
      ...
      (error, results) => {
        ...
        var context = {
          ...
          pNum: pNum,
          re_YN: re_YN,
          hasReply: results[2][0].replyCount > 0,
        };
  ...
  answer: (req, res) => {
    ...
    if (loginInfo.cls !== "MNG") {
      return res.send(
        `<script>alert("관리자 권한이 없습니다."); history.back();</script>`,
      );
    }

    var sql1 = `SELECT * FROM boardtype; `;
    var sql2 = `SELECT b.*, p.name AS writer_name FROM board b LEFT JOIN person p ON b.loginid = p.loginid WHERE b.board_id = ?; `;
  ...
  answer_process: (req, res) => {
    var loginInfo = authIsOwner(req, res);
    if (!loginInfo || loginInfo.cls !== "MNG") {
      return res.send(
    ...
    var post = req.body;
    var original_bid = post.board_id;
    var pNum = post.pNum;
    var answer_content = sanitizedHtml(post.answer_content);

    db.query(
      `SELECT type_id, title, p_id FROM board WHERE board_id = ?`,
      [original_bid],
      (error, results) => {
        if (error) throw error;
        var orig = results[0];

        var reply_title = `[답변] : ${orig.title}`;
        ...
        var sql = `
        INSERT INTO board (type_id, loginid, title, content, date, p_id, password) 
        VALUES (?, '관리자', ?, ?, ?, ?, '1234');
      `;

        db.query(
          sql,
          [orig.type_id, reply_title, answer_content, dateStr, orig.p_id],
          (err, result) => {
            if (err) throw err;
            res.redirect(`/board/view/${orig.type_id}/${pNum}`);
          },
        );
      },
    );
